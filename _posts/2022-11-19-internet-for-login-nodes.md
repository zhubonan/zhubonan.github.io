...
Luckily, there is a simple way to bypass this limitation without hacking through the firewall - SOCKS5 proxy with `ssh` [reverse tunnelling](https://man.openbsd.org/ssh).
...
I have not extensively tested this, but it seems that `wget` does not work with SOCKS5 at all, but `curl` does.
...
For Julia, both the two ways above work with `Pkg` as of 1.8.
