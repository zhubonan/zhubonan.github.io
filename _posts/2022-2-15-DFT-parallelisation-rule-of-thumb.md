...
- do not over parallelise by using too many cores - you can end on the wrong side of the scaling curve
...
However, there are things to watch out for and one needs to understand how parallelisation works in order to maximise the efficiency.

While sensible "defaults" are often described in the documentation or chosen by the code itself, there is one thing that the code cannot choose - the number of MPI processors to be used.
...
This explains the inevitable drop of the speed-up vs num-of-cpus plot, as the latter becomes more and more the dominant contribution to the run time.
In addition, parallelisation also has certain overheads, each processor needs to communicate with others in some ways, and the communication cost may increase with increasing number of cores.
This is the cause of the drop in the efficiency with high core counts in many cases.
Even if the communication cost does not scale with the number of cores, its contribution to the "parallelised" part still increases with the number of processors because the time of doing actual work becomes less and less.

Second, let's briefly discuss how plane wave DFT codes are parallelised.
...
The parallelisation is commonly conducted at three levels:
...
The kpoint parallelisation is the most efficient one, exploiting the fact that each kpoint is almost independent of each other - they only talk to each other via the electron density (which is not the case any more in hybrid DFT).
...
Unlike the other two levels, parallelisation over kpoint does not distribute the memory, each process still receives the full set of kpoints.
This is not the case with band and plane wave parallelisation, however, these two do involve heavy ("all-to-all") communications, making it more and more difficult to scale well on large core counts.
On the other hand, one can expect almost perfect scaling with k-point parallelisation.
There are of course many fine details on this topic, and many other parts of the code (for example, auxiliary arrays and ionic solvers) can cost memory.
...
A bit on the terminology - the number of _kpoint/band groups_ means that MPI processes are divided into $N$ groups for this level, and each group may contain $M$ number of processes.
...
For VASP, this information can be obtained at the top of the `OUTCAR`:
...
In this calculation there are 48 bands, hence each band group has 12 bands to work on.
...
While these calculations are easier to scale purely as a result of them being very compute-heavy (they scale to thousands of cores, while most GGA calculation won't), careful tuning of parallelisation becomes even more important as every bit of performance gain can lead to huge savings of resources.
...
Also note the speed up by doubling the core-count - we achieve an 80% gain in speed by throwing twice the computing resource - not bad at all.

## Further topics

We have not touched OpenMP parallelisation and GPU parallelisation.
...
Nevertheless, manual tuning can be valuable for large-scale calculations to achieve good performance.
